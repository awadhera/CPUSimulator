...
void instParse(std::vector< std::vector< std::map< int, std::map< int, int > > > > &renInst, std::vector< std::vector<int> > &allInst, std::map<int, int> &instMap, std::map<int, int> &prs, int &winSize, int &execUnits)
{
	int dep = 0;
	int count = 0;
...
int main(int argc, char *argv[])
{
	...
	int winSize = *argv[2] - '0';
	int execUnits = *argv[3] - '0';
	if (winSize < execUnits)
	{
		std::cout << "Window size is less than number of execution units...Exiting\n";
		std::exit(0);
	}
	...
	int availExec = execUnits;
	do
	{
		instParse(renInst, allInst, instMap, prs, winSize, availExec);
