...
void instParse(std::vector< std::vector< std::map< int, std::map< int, int > > > > &renInst, std::vector< std::vector<int> > &allInst, std::map<int, int> &instMap, std::map<int, int> &prs, int &winSize)
{
	int dep = 0;
	int count = 0;
...
int main(int argc, char *argv[])
{
	...
	do
	{
		instParse(renInst, allInst, instMap, prs, winSize);
